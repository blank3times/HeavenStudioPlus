    ...
    public static class PcoCocoaBarLoader
    {
        public static Minigame AddGame(EventCaller eventCaller)
        {
            return new Minigame("cocoaBar", "Cocoa Bar [Tentative]", "211571", false, false, new List<GameAction>()
            {
                ...
                {
                    function = delegate { CocoaBar.instance.Pancake(eventCaller.currentEntity.beat); },

                    defaultLength = 4f
                },
                new GameAction("cocoa", "Hot Cocoa")
                {
                    function = delegate { CocoaBar.instance.Cocoa(eventCaller.currentEntity.beat); },
...
namespace HeavenStudio.Games
{
    ...
    public class CocoaBar : Minigame
    {
        ...
        public void Pancake(double beat)
        {
            ScheduleInput(beat, 2f, InputAction_BasicPress, PlateSuccess, ServeMiss, Nothing);
            ScheduleInput(beat, 2.75f, InputAction_BasicPress, PlateSuccess, ServeMiss, Nothing);
            ScheduleInput(beat, 3.5f, InputAction_BasicPress, PlateSuccess, ServeMiss, Nothing);            
            MultiSound.Play(new MultiSound.Sound[] {
                new MultiSound.Sound("cocoaBar/ding_temp", beat),
                new MultiSound.Sound("cocoaBar/customer_temp", beat + 1f),
                new MultiSound.Sound("cocoaBar/customer_temp", beat + 1.5f)
            });
        }

        public void Cocoa(double beat)
        {
            ScheduleInput(beat, 1.5f, InputAction_AltPress, FillSuccess, ServeMiss, Nothing);
            ScheduleInput(beat, 2.5f, InputAction_AltRelease, CupSuccess, ServeMiss, Nothing);
            MultiSound.Play(new MultiSound.Sound[] {
                new MultiSound.Sound("cocoaBar/ding_temp", beat),
